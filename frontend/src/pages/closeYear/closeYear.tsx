...
function CloseYear() {
    ...
    const {queryOnLoad, mutationPatchYear, mutationUpdateStudent, allStudents} = useDirector()
     
    const onSubmit = ()=>{
      if(queryOnLoad.data?.year.year)mutationPatchYear.mutate(queryOnLoad.data?.year)
    }
    ...
    {queryOnLoad.isInitialLoading?<Spinner/> : 
      <>
        <Header />
